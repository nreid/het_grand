...
lratxy <- function(it,gt){
	...
	lmal <- dbinom(x=sum(gt[it=="M"]),size=sum(it=="M"),prob=0.5,log=TRUE)
	l <- c(null=lnullf+lnullm,alt=lfem+lmal)
	diff(l)
...
lratzw <- function(it,gt){
	...
	lfem <- dbinom(x=sum(gt[it=="F"]),size=sum(it=="F"),prob=0.5,log=TRUE)
	l <- c(null=lnullf+lnullm,alt=lfem+lmal)
	diff(l)
...
while(length(line <- readLines(f,n=1)) > 0) {
	...
	pa <- c(lratxy(it=gs,gt=gt), lratzw(it=gs,gt=gt))
	pa <- c(line[1],line[2],table(gs),pa)
