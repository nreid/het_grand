...
# use table of deletion genotypes in grandis to create new "populations"
sam <- cname[10:585] %>% gsub("\\.","_",.) %>% gsub("_.$","",.)
del <- read.table("~/het_grand/grandis_del_gt.txt",stringsAsFactors=FALSE)
dpop <- pop
dpop[sam %in% del[del[,3]==0,1]] <- "d0"
dpop[sam %in% del[del[,3]==1,1]] <- "d1"
dpop[sam %in% del[del[,3]==2,1]] <- "d2"
dpopu <- unique(dpop)
dpopu <- sort(dpopu)

slist <- lapply(sort(popu), FUN=function(x){which(pop==x)})
dslist <- lapply(sort(dpopu), FUN=function(x){which(dpop==x)})
...
while(length(line <- readLines(f,n=1)) > 0) {
	...
	# list of vectors of haploid genotypes per population, or deletion category
	sgt <- lapply(slist,FUN=function(x,v){x <- v[x]; x[!is.na(x)]},v=gt)
	dsgt <- lapply(dslist,FUN=function(x,v){x <- v[x]; x[!is.na(x)]},v=gt)

	# if sample size doesn't equal at least vector, skip
	minsam <- c(5,12,3,6,5,5,5,5,5)
	samsize <- sapply(dsgt,length)
	...
	# caculate pi within
	pis <- sapply(dsgt,piw,d=d) %>% round(.,digits=5)
	pibs <- apply(t(combn(dsgt,2)),MAR=1,FUN=function(x){pib(d=d,x[[1]],x[[2]])})  %>% round(.,digits=5)
