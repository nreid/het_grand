...
# calculate the proportion of allelic comparisons that can be called
	# shared polymorphism
	# e.g. 2*p1*q1 * 2*p2*q2

sharedp <- function(d,alvec1,alvec2){

	# counts of alleles
	ac1 <- alvec1 %>% factor(.,levels=1:dim(d)[1]) %>% table()
	ac2 <- alvec2 %>% factor(.,levels=1:dim(d)[1]) %>% table()

	if(length(ac1)==1 | length(ac2)==1){return(0)}

	# counts of pairwise comparisons among alleles
	fc1 <- (ac1 %>% combn(.,2) %>% apply(.,MAR=2,FUN=prod))
	fc2 <- (ac2 %>% combn(.,2) %>% apply(.,MAR=2,FUN=prod))

	# vector of probabilities of each pairwise comparison (heterozygosites of each allele pair)
	p1 <- (fc1)/choose(length(alvec1),2)
	p2 <- (fc2)/choose(length(alvec2),2)

	# probability of being heterozygous for a given pair in both populations weighted by number of differences between alleles
	p <- sum(p1*p2*d[lower.tri(d)])
...
while(length(line <- readLines(f,n=1)) > 0) {
	...
	pibs <- apply(t(combn(sgt,2)),MAR=1,FUN=function(x){sharedp(d=d,x[[1]],x[[2]])})  %>% round(.,digits=5)
